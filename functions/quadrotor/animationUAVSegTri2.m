...
% for seg-triMesh ONLY
% with the given name of the video
% 
% Benji Z. Zhang
% 2023/1

function animationUAVSegTri2(ax,robpts,trajALL,trajId,tri,tm_via,g,area,videoName)
    ...
    for curT = tm_via(1):0.02:tm_via(2)
        ...
        triMeshY = tri(:,[2 5 8])';
        triMeshZ = tri(:,[3 6 9])';
        % patch(ax, triMeshX,triMeshY,triMeshZ,[.8,.8,.8],'EdgeColor','red','FaceAlpha',1,'LineWidth',1)      
        patch(ax, triMeshX,triMeshY,triMeshZ,[.8,.8,.8],'EdgeColor','k','FaceAlpha',0.6,'LineWidth',1)        
